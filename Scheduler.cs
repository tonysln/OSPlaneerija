    ...
    class Scheduler
    {
        ...
        public void FCFS()
        {
            ...
            for (int tick = 0, i = 0; tick <= 50; tick++)
            {
                ...
                // Now it should start running, set the start tick (only if it hasn't run before)
                current.Start = (current.Running || current.Remaining != current.BurstTime) ? current.Start : tick;
        ...
        public void SRTF()
        {
            ...
            for (int tick = 0, i = 0; tick <= 50; tick++)
            {
                ...
                for (int j = i + 1; j < TaskList.Count; j++)
                {
                    ...
                    // If there is a new task available and its burst time is shorter than current,
        ...
        {
            // Every task <= quant (req: 3) ticks long gets completed right away,
            // all other tasks get cut off after n ticks and sent to the LAST position in the array,
        ...
        public void DoubleFCFS()
        {
            ...
            // All tasks with <= 6 ticks get into the high priority queue
            ...
            for (int tick = 0, i = 0; tick <= 50; tick++)
            {
                if (safe > 50)
                {
                    // Whooops, something went really wrong.
                    TaskHistory.Add(new Task(0, 0, "P0", Color.White));
                    return;
                }
                ...
                // Detect gaps (no task at the current tick)
                if (!current.Running && current.Elapsed == 0 && current.ArrivalTime > tick)
                {
                    TaskHistory.Add(new Task(0, 0, "P0", Color.White)); // Add an empty task
                    continue;
                }
                ...
                if (current.Running)
                {
                    ...
                    for (int j = i + 1; j < TaskList.Count; j++)
                    {
                        ...
                        if (current.BurstTime > 6 && other.ArrivalTime <= tick && other.BurstTime <= 6)
                        {
                            TaskList.Insert(i, other);
                            TaskList.RemoveAt(j + 1); // j + 1 because now the list is (n+1) and items have shifted
                            current.Running = false;
                ...
                // Detect empty tasks at the start
                if (!current.Running && current.Elapsed == 0)
                {
                    TaskHistory.Add(new Task(0, 0, "P0", Color.White)); // Add an empty task
                    continue;
                }
